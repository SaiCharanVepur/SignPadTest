var wacomstu540 = function () {
    ...
    this.config = {
        ...
        pressureFactor: 1023,
        onPenDataCb: null,
        onHidChangeCb: null, // Callback for HID changes (including disconnect)
    };
    ...
    this.disconnect = async function () {
        if (this.device && this.device.opened) {
            ...
            if (this.config.onHidChangeCb) {
                this.config.onHidChangeCb('disconnect', this.device);
            }
        }
    }.bind(this);

    this.handleDisconnect = function (event) {
        if (event.device === this.device) {
            console.log("Device was disconnected.");
            this.device = null;
            if (this.config.onHidChangeCb) {
                this.config.onHidChangeCb('disconnect', event.device);
            }
        }
    }.bind(this);
